...
module.exports = class extends Command {
	...
	async run(msg) {
		await msg.channel.send('Capturing HEAP Snapshot. This may take a while... HEAP Snapshot will be sent in your DMs!');

		// Capture the snapshot (this freezes the entire VM)
		const path = join(process.cwd(), `${Date.now()}.heapsnapshot`);
		await writeSnapshot(path);

		return msg.author.send(`Captured in \`${path}\`, check! Remember, do NOT share this with anybody, it may contain a lot of sensitive data.`);
	}
